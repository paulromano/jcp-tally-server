...
we have $d$ bytes where $d \ge 8$ stems from the fact that a single scoring
event may need to score to hundreds of scoring functions. The message sent to a
server at a scoring event consists of the scores for each scoring function.
...
\cite{bluegene}. Thus, we can infer that $\alpha = 3.53 \cdot 10^{-6} \unit{s}$
and $\beta = 2.60 \cdot 10^{-9} \unit{s/byte}$ for the Intrepid Blue Gene/P
supercomputer. For the Monte Carlo Performance Benchmark modified to use 320
nuclides in the fuel, our tests using OpenMC show that Blue Gene/P can simulate
about 76 particles per second on each processor, i.e. $\mu = 0.0132
\unit{s/particle}$. We have chosen to use 320 nuclides in the fuel as this is
the maximum number of nuclides available in the ENDF/B-VII.0 cross section
...
Cray Gemini interconnect which has higher latency but faster bandwidth than the
interconnect on the Blue Gene/P. The parameters we use here are $1/\mu = 140$,
$\alpha = 1.4 \cdot 10^{-5} \unit{s}$ and $\beta = 1.0 \cdot 10^{-9}
\unit{s/byte}$ (NEED REFERENCE/UPDATED DATA FOR TITAN).
...
21.3$ and $d = 15360$, the model predicts an overhead of between 10 and 20
percent of the total running time for both the Intrepid Blue Gene/P and Titan
Cray XK6 supercomputers when using blocking communication. If non-blocking
communication is used, the overhead is expected to be less than 10 percent.

\begin{table}[htb]
  \ttabbox[\FBwidth]{
  \caption{Parameters used for tally server overhead model in
    \eqref{eq:model-blocking}.}
...
server for every 12 compute processors for the same values of $f$ and $d$.
...
\begin{figure*}[!tbh]
  ...
  \begin{floatrow}[2]
    \ffigbox[\FBwidth] {
          \caption{Tally server overhead on Titan Cray XK6 as a function
            of data per event with $c/s = 1$.}
          \label{fig:titan-r1}
