...
each scoring event would have to increment 300 tally bins. We see that the fact
that we have $d$ bytes where $d \ge 8$ (the size of one double-precision float)
stems from the fact that a single scoring event may need to score to hundreds of
